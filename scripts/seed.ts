  ...
  const statusLabels = [
    {
      ...
      hideInSearchFilters: true,
      hideInItemDetails: false,
      displayInGroupPage: false,
      auto_display: true,
      allow_stolen_api: false,
      prevent_transfer: false,
      prevent_audit: false,
      prevent_upload_to_live: false,
    },
    {
      legacyId: 2625,
      name: "Live",
    ...
    {
      legacyId: 2636,
      name: "Pending Visibility - Single Count",
      includeInInventory: false,
      hideInSearchFilters: true,
      hideInItemDetails: false,
      displayInGroupPage: false,
    ...
    {
      legacyId: 2637,
      name: "Pending Visibility - Unknown",
      includeInInventory: true,
      hideInSearchFilters: true,
      hideInItemDetails: false,
      displayInGroupPage: false,
  ...
  for (const row of statusLabels) {
    await pool.query(
      `INSERT INTO status_labels (
         ...
         auto_display, allow_stolen_api, prevent_transfer, prevent_audit, prevent_upload_to_live
       )
       ...
       ON CONFLICT (name) DO UPDATE SET
         ...
         auto_display = EXCLUDED.auto_display,
         allow_stolen_api = EXCLUDED.allow_stolen_api,
         prevent_transfer = EXCLUDED.prevent_transfer,
         prevent_audit = EXCLUDED.prevent_audit,
         prevent_upload_to_live = EXCLUDED.prevent_upload_to_live,
         updated_at = now()`,
      [
        row.legacyId,
        row.name,
        "",
        row.includeInInventory,
        row.hideInSearchFilters,
        row.hideInItemDetails,
        row.displayInGroupPage,
        row.auto_display,
        row.allow_stolen_api,
        row.prevent_transfer,
        row.prevent_audit,
        row.prevent_upload_to_live,
      ],
    );
  }
