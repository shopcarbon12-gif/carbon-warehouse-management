/**
 * When Coolify Postgres public port times out from your PC, forward over SSH then run
 * `ensure-orlando-seed-bins.mjs` against the tunneled URL.
 *
 *   node scripts/ensure-orlando-bins-through-tunnel.mjs
 *   node scripts/ensure-orlando-bins-through-tunnel.mjs -- --dry-run
 *   node scripts/ensure-orlando-bins-through-tunnel.mjs -- --tenant-slug=cj --location-code=001
 *
 * Env: same as copy-bins-through-tunnel (COOLIFY_SSH_HOST, COOLIFY_SSH_USER, ports).
 */
...
if (!targetTemplate || !targetTemplate.startsWith("postgresql")) {
  console.error("DATABASE_URL missing in .env.coolify.local.");
  process.exit(1);
}
...
const remotePort = Number(
  process.env.COOLIFY_SSH_REMOTE_PG_PORT?.trim() || coolify.COOLIFY_POSTGRES_PUBLIC_PORT || "3000",
);
...
ssh.on("error", (e) => {
  ...
  console.error("Install OpenSSH client (Windows Optional Feature) or use Git Bash.");
  process.exit(1);
});
...
const child = spawn(process.execPath, ["scripts/ensure-orlando-seed-bins.mjs", ...passThrough], {
  cwd: root,
  env: { ...process.env, ENSURE_BINS_DATABASE_URL: tunneledTarget },
  stdio: "inherit",
  shell: false,
});
