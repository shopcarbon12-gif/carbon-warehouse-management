...
class WmsApiClient {
  ...
  /// `GET /api/locations/bins` — list all bins (returns [{id, code, ...}]).
  Future<List<Map<String, dynamic>>> fetchBins() async {
    ...
    if (decoded is Map<String, dynamic>) {
      final rows = decoded['bins'] ?? decoded['rows'] ?? decoded['data'];
      if (rows is List) return rows.whereType<Map<String, dynamic>>().toList();
    }
    return [];
  }

  /// `GET /api/locations/bins/contents?binId=` — contents of a bin (EPCs + SKU roll-up).
  /// Returns {items: [{sku, description, customSkuId, qty, epcs:[...]}]}
  Future<Map<String, dynamic>> fetchBinContents(String binId) async {
    final base = (await resolveBaseUrl()).replaceAll(RegExp(r'/+$'), '');
  ...
  /// `POST /api/locations/bins` — create a new bin. Body: {code}.
  /// Returns the created bin object {id, code, ...}.
  ...
  /// `GET /api/inventory/catalog?view=grid&q=` with a partial-SKU query
  /// that fetches all matching matrix rows. Returns the raw rows list.
  Future<List<Map<String, dynamic>>> fetchCatalogMatrixRows(String q) async {
    final qt = q.trim();
    if (qt.isEmpty) return [];
    final base = (await resolveBaseUrl()).replaceAll(RegExp(r'/+$'), '');
    final uri = Uri.parse('$base/api/inventory/catalog').replace(
      queryParameters: {'view': 'grid', 'page': '1', 'limit': '50', 'q': qt},
    );
    final res = await _http.get(uri, headers: await sessionAuthHeaders());
    if (res.statusCode < 200 || res.statusCode >= 300) return [];
    final decoded = jsonDecode(res.body);
  ...
  /// `GET /api/inventory/catalog?matrixId=` — all custom SKUs for a product matrix.
  /// Returns list of custom SKU objects [{id, sku, color, size, description, ...}].
  Future<List<Map<String, dynamic>>> fetchCatalogByMatrixId(String matrixId) async {
    if (matrixId.isEmpty) return [];
    final base = (await resolveBaseUrl()).replaceAll(RegExp(r'/+$'), '');
    final uri = Uri.parse('$base/api/inventory/catalog').replace(
      queryParameters: {'matrixId': matrixId},
    );
    final res = await _http.get(uri, headers: await sessionAuthHeaders());
    if (res.statusCode < 200 || res.statusCode >= 300) return [];
    final decoded = jsonDecode(res.body);
    ...
    if (decoded is Map<String, dynamic>) {
      final rows = decoded['rows'] ?? decoded['items'] ?? decoded['data'];
      if (rows is List) return rows.whereType<Map<String, dynamic>>().toList();
    }
    return [];
  }

  /// `GET /api/rfid/epcs-for-sku?sku=` — EPC list for a specific custom SKU.
  /// Returns [{epc, status, ...}].
  Future<List<Map<String, dynamic>>> fetchEpcsForCustomSku(String sku) async {
    if (sku.isEmpty) return [];
    final base = (await resolveBaseUrl()).replaceAll(RegExp(r'/+$'), '');
    final uri = Uri.parse('$base/api/rfid/epcs-for-sku').replace(
      queryParameters: {'sku': sku.trim()},
    );
    final res = await _http.get(uri, headers: await sessionAuthHeaders());
    ...
    if (decoded is List) return decoded.whereType<Map<String, dynamic>>().toList();
    if (decoded is Map<String, dynamic>) {
      final items = decoded['epcs'] ?? decoded['items'] ?? decoded['data'];
      if (items is List) return items.whereType<Map<String, dynamic>>().toList();
    }
    return [];
  }

  /// `DELETE /api/locations/bins/contents` — remove a specific SKU from a bin.
  /// Body: {binId, sku}. Falls back to clean-and-reassign if endpoint absent.
  Future<void> removeSkuFromBin({required String binId, required String sku}) async {
    final base = (await resolveBaseUrl()).replaceAll(RegExp(r'/+$'), '');
    final uri = Uri.parse('$base/api/locations/bins/contents');
    final res = await _http.delete(
      ...
      body: jsonEncode({'binId': binId, 'sku': sku.trim()}),
    );
    if (res.statusCode < 200 || res.statusCode >= 300) {
      throw WmsApiException(res.statusCode, res.body);
    }
