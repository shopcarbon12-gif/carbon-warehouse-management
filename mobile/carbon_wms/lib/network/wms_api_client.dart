...
class WmsApiClient {
  ...
  /// Transfer slip report rows for a direction ('out' | 'in'). Server returns
  /// created_by_name / received_by_name as "First L.".
  /// `GET /api/reports/transfers?direction=`
  Future<List<Map<String, dynamic>>> fetchTransferReport({
    String direction = 'out',
    String q = '',
  }) async {
    final base = (await resolveBaseUrl()).replaceAll(RegExp(r'/+$'), '');
    final uri = Uri.parse('$base/api/reports/transfers').replace(
      queryParameters: {
        'direction': direction,
        if (q.trim().isNotEmpty) 'q': q.trim(),
      },
