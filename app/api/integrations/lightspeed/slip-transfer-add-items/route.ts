...
export async function POST(req: Request) {
  ...
  const batches = chunkTransferItems(parsed.data.items);
    ...
    if (!result.ok) {
      try {
        await insertExternalSystemLog(pool, session.tid, {
          ...
          direction: "OUTBOUND",
          payload_summary: `Inventory/Transfer/${lsTid}/AddItems batch=${b} http=${result.status} slip=${parsed.data.slipNumber} (partial failure)`,
          status: "error",
        });
      } catch (e) {
  ...
  try {
    await insertExternalSystemLog(pool, session.tid, {
      system_name: "lightspeed",
      direction: "OUTBOUND",
      payload_summary: `Inventory/Transfer/${lsTid}/AddItems batches=${batches.length} lines=${parsed.data.items.length} slip=${parsed.data.slipNumber}`,
      status: "ok",
    });
  } catch (e) {
    console.warn("[slip-transfer-add-items] log", e);
  ...
  return NextResponse.json({
    ...
    slipNumber: parsed.data.slipNumber,
    ls_transfer_id: lsTid,
    lines_added: parsed.data.items.length,
    batchResults,
    lightspeed: lastBody,
  });
}
